...
public class TokenServiceJWTImpl implements TokenService {
	...
	private void cacheLoginUser(LoginUser loginUser) {
		/* 如果该用户已登录，则删除redis中该用户token，之前登录失效 */
		if(map!=null && (map.containsKey(loginUser.getUsername()))){
			redisTemplate.delete("tokens:"+(String) map.get(loginUser.getUsername()));
		}
		map.put(loginUser.getUsername(),loginUser.getToken()); //map放入新登录用户
